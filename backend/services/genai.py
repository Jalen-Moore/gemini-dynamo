...
class YoutubeProcessor:
    ...
    def find_key_concepts(self, documents : list, sample_size : int = 0, verbose = False):
        ...
        # Find number od documents in each group
        ...
        for group in tqdm(groups):
            ...
            prompt = PromptTemplate(
                ...
                You must respond as a JSON object without any backticks. The JSON object must separate each concept with a comma, and each concept is defined with the following structure:
                {{
                    "term": "definition"
                }}                
                """,
            ...
            # Create chain
            chain = prompt | self.genai_processor.model | parser
            ...
            try:
                ...
                logger.info(f"Failed to find concepts for group {e}")
            ...
            if verbose:
                ...
                total_output_characters = len(output_concept)
        ...
        # Convert each JSON string in batch_concepts to a Python dictionary 
        # processed_concepts = [json.loads(concept) for concept in batch_concepts]

        logging.info(f"Total Analysis Cost: ${batch_cost}")

        logger.info(batch_concepts)
        formatted_concepts = self.format_processed_concepts(batch_concepts)
        logger.info(formatted_concepts)
